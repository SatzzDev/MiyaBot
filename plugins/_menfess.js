...
const {Handler, Miya} = require('../lib/command')

Handler(async(m, {miya, user, budy, command, react, qmsg, reply}) => {
//━━━━━━━━━━━━━━━[ MENFESS RESPONSE ]━━━━━━━━━━━━━━━━━//
...
miya.sendText(m.chat, '```Anda telah meninggalkan Chat Anonim```', global.fake);
let other = room.other(m.sender);
await miya.sendText(other, '```Lawan Chat Anda telah meninggalkan Chat Anonim```', global.fake);
...
await miya.sendMessage(OWNER_NUMBER, { text: `PESAN DARI MENFESS @${room.a.split('@s.whatsapp.net')[0]} dan @${room.b.split('@s.whatsapp.net')[0]}`, mentions: [room.a, room.b] },{quoted:m});
}
}
...
if (m.sender == room.b) {
if (INI == "TERIMA") {
miya.sendButtons(m.chat, `( *\`MENFESS\`* )`, "```Chat Anonim Telah Terhubung\nTekan Tombol Di Bawah Untuk Menghentikan Chat Secara Anonim.```", '', [{ type: 'btn', text: 'KELUAR', id: 'keluar' }], global.fake);
room.state = "ACTIVE";
let other = [room.a, room.b].find((user) => user !== m.sender);
//miya.sendMessage(other, { text: "```Penerima telah mengkonfirmasi, Chat Anonim telah terhubung```", });
miya.sendButtons(other, `( *\`MENFESS\`* )`, "```Penerima telah mengkonfirmasi, Sekarang Anda Dan Target Dapat Bertukar Pesan Secara Anonim Telah Terhubung.```", '', [{ type: 'btn', text: 'KELUAR', id: 'keluar' }], global.fake);
}
if (INI == "TOLAK") {
m.reply("Okay.");
let other = [room.a, room.b].find((user) => user !== m.sender);
miya.sendText(other, "```Penerima menolak Permintaan anda untuk melakukan Chat secara Anonim.```", global.fake);
room.state = "CANCELED";
delete global.db.data.menfess[room.id];
}
}
...
var checkTarget = await miya.onWhatsApp(numberTarget);
...
miya.sendText(m.chat, '```Menunggu konfirmasi dari target...```', global.fake);
miya.sendButtons(numberTarget, `( *\`MENFESS\`* )`, '```Seseorang ingin bertukar pesan secara anonim kepada Anda. Tekan Tombol di bawah untuk menerima/menolak```', '', [{ type: 'btn', text: 'TERIMA', id: 'accept' },{ type: 'btn', text: 'TOLAK', id: 'reject' }], global.fake);
/*miya.sendMessage(numberTarget, {
text: `Halo kak ${await miya.getName(numberTarget)}\n` + '```Seseorang ingin bertukar pesan secara anonim kepada Anda. Tekan Tombol di bawah untuk menerima/menolak```',
contextInfo: {
externalAdReply: {
title: "MENFESS",
body: calender,
