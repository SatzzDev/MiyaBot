...
const { generateWAMessageContent, generateWAMessageFromContent, proto } = require('baileys');
async function sendCardCarousel(chatId, title, text, footer, cards = [], quoted) {
const results = [];
for (const card of cards) {
console.log(card)
const { imageMessage } = await generateWAMessageContent({image: {url: card.image}}, {upload: miya.waUploadToServer});
results.push({
...
await miya.relayMessage(chatId, messageContent.message, {'messageId': messageContent.key.id});
}
